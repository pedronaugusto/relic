//! A TLS 1.2 and 1.3 client: the standard library's `std.crypto.tls.Client`,
//! with client authentication added. A server that asks for a certificate
//! with a CertificateRequest is answered with the chain and key in
//! `Options.client_auth` — a Certificate and a CertificateVerify signed
//! with RSA-PSS, RSA PKCS #1 v1.5 (TLS 1.2 only), ECDSA on P-256 or P-384,
//! or Ed25519, whichever of the server's schemes the key signs with first —
...
pub const Options = struct {
    ...
    /// with. Without it, a server that asks is sent an empty Certificate,
    /// and decides for itself whether to go on.
    client_auth: ?*const ClientAuth = null,
    /// Set to true when the server asks for a certificate, so a caller can
    /// tell a refusal of one from another failure.
    certificate_requested: ?*bool = null,

    /// How many bytes `entropy` holds: the handshake's, then 64 for a
    /// signature's salt or nonce.
...
pub const InitError = error{
    ...
    /// schemes it takes is one `Options.client_auth`'s key signs with.
    ClientCertificateSchemeUnsupported,
    /// The client's key does not make signatures: numbers that are not an
    /// RSA key.
    ClientKeyInvalid,
    /// A signature by the client's key did not verify with it, and was not
    /// sent.
    ClientSignatureFault,
    OutOfMemory,
    DiskQuota,
    LockViolation,
    NotOpenForWriting,
...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    // A server's CertificateRequest: whether one came, its context (TLS
    // 1.3), and the scheme it is answered in.
    var cert_requested = false;
    var cert_request_context_buf: [255]u8 = undefined;
    var cert_request_context: []const u8 = &.{};
    var client_scheme: ?tls.SignatureScheme = null;
    const sign_entropy = options.entropy[240..304];
    // TLS 1.2's CertificateVerify signs the handshake's messages
    // themselves, not a hash of them: they are kept while there is a key
    // to sign them with.
    var raw_transcript: std.ArrayList(u8) = .empty;
    defer if (options.client_auth) |auth| raw_transcript.deinit(auth.gpa);
            ...
            .handshake => while (true) {
                ...
                switch (handshake_type) {
                    .server_hello => {
                        ...
                        switch (cipher_suite_tag) {
                            ...
                            => |tag| {
                                ...
                                if (tls_version == .tls_1_2) try keepRaw(&raw_transcript, options.client_auth, &.{
                                    cleartext_header[tls.record_header_len..], host, wrapped_handshake,
                                });
                            },
                    ...
                    .certificate => cert: {
                        ...
                        if (tls_version == .tls_1_2) try keepRaw(&raw_transcript, options.client_auth, &.{wrapped_handshake});
                    ...
                    .server_key_exchange => {
                        ...
                        try keepRaw(&raw_transcript, options.client_auth, &.{wrapped_handshake});
                    ...
                    .certificate_request => {
                        if (cert_requested) return error.TlsUnexpectedMessage;
                        const request: CertificateRequest = switch (tls_version) {
                            .tls_1_3 => request: {
                                if (cipher_state != .handshake) return error.TlsUnexpectedMessage;
                                if (handshake_state != .certificate) return error.TlsUnexpectedMessage;
                                break :request try .parse13(hsd.rest());
                            },
                            .tls_1_2 => request: {
                                if (cipher_state != .cleartext) return error.TlsUnexpectedMessage;
                                if (handshake_state != .server_hello_done) return error.TlsUnexpectedMessage;
                                try keepRaw(&raw_transcript, options.client_auth, &.{wrapped_handshake});
                                break :request try .parse12(hsd.rest());
                            },
                            else => unreachable,
                        };
                        switch (handshake_cipher) {
                        ...
                        @memcpy(cert_request_context_buf[0..request.context.len], request.context);
                        cert_request_context = cert_request_context_buf[0..request.context.len];
                        cert_requested = true;
                        if (options.certificate_requested) |flag| flag.* = true;
                        if (options.client_auth) |auth| {
                            client_scheme = request.choose(auth.key.schemes(tls_version == .tls_1_2)) orelse
                                return error.ClientCertificateSchemeUnsupported;
                        }
                    },
                    .server_hello_done => {
                        if (tls_version != .tls_1_2) return error.TlsUnexpectedMessage;
                        ...
                        // Asked for a certificate, the client sends its
                        // Certificate before the key exchange and its
                        // CertificateVerify after.
                        var certificate_head: [7]u8 = undefined;
                        const client_list: []const u8 = if (options.client_auth) |auth| auth.list12 else &.{};
                        if (cert_requested) {
                            certificate_head[0] = @intFromEnum(tls.HandshakeType.certificate);
                            mem.writeInt(u24, certificate_head[1..4], @intCast(3 + client_list.len), .big);
                            mem.writeInt(u24, certificate_head[4..7], @intCast(client_list.len), .big);
                        }
                        var verify_head: [8]u8 = undefined;
                        var signature_buf: [PrivateKey.max_signature_len]u8 = undefined;
                        ...
                        switch (handshake_cipher) {
                            inline else => |*p| {
                                ...
                                try keepRaw(&raw_transcript, options.client_auth, &.{wrapped_handshake});
                                if (cert_requested) {
                                    p.transcript_hash.update(&certificate_head);
                                    p.transcript_hash.update(client_list);
                                    try keepRaw(&raw_transcript, options.client_auth, &.{ &certificate_head, client_list });
                                }
                                p.transcript_hash.update(client_key_exchange_prefix[tls.record_header_len..]);
                                p.transcript_hash.update(public_key_bytes);
                                if (cert_requested) if (options.client_auth) |auth| {
                                    try keepRaw(&raw_transcript, options.client_auth, &.{ client_key_exchange_prefix[tls.record_header_len..], public_key_bytes });
                                    const scheme = client_scheme.?;
                                    signature = try signWith(auth.key, scheme, raw_transcript.items, sign_entropy, &signature_buf);
                                    verify_head[0] = @intFromEnum(tls.HandshakeType.certificate_verify);
                                    mem.writeInt(u24, verify_head[1..4], @intCast(4 + signature.len), .big);
                                    mem.writeInt(u16, verify_head[4..6], @intFromEnum(scheme), .big);
                                    mem.writeInt(u16, verify_head[6..8], @intCast(signature.len), .big);
                                    p.transcript_hash.update(&verify_head);
                                    p.transcript_hash.update(signature);
                                };
                                ...
                                if (cert_requested) try writePlainHandshake(output, &.{ &certificate_head, client_list });
                                try output.writeAll(&client_key_exchange_prefix);
                                try output.writeAll(public_key_bytes);
                                if (signature.len != 0) try writePlainHandshake(output, &.{ &verify_head, signature });
                        ...
                        const app_cipher = app_cipher: switch (handshake_cipher) {
                            inline else => |*p, tag| switch (tls_version) {
                                .tls_1_3 => {
                                    ...
                                    // The application's secrets come from the
                                    // handshake up to the server's Finished;
                                    // the client's Finished covers its own
                                    ...
                                    var handshake_seq: u64 = 0;
                                    if (cert_requested) {
                                        var sealer: Sealer13(P) = .{
                                            .output = output,
                                            .key = pv.client_handshake_key,
                                            .iv = pv.client_handshake_iv,
                                            .seq = &handshake_seq,
                                        };
                                        const client_list: []const u8 = if (options.client_auth) |auth| auth.list13 else &.{};
                                        var head: [4 + 1 + 255 + 3]u8 = undefined;
                                        const ctx = cert_request_context;
                                        head[0] = @intFromEnum(tls.HandshakeType.certificate);
                                        mem.writeInt(u24, head[1..4], @intCast(1 + ctx.len + 3 + client_list.len), .big);
                                        head[4] = @intCast(ctx.len);
                                        @memcpy(head[5..][0..ctx.len], ctx);
                                        mem.writeInt(u24, head[5 + ctx.len ..][0..3], @intCast(client_list.len), .big);
                                        const certificate_head = head[0 .. 5 + ctx.len + 3];
                                        p.transcript_hash.update(certificate_head);
                                        p.transcript_hash.update(client_list);
                                        try sealer.add(certificate_head);
                                        try sealer.add(client_list);
                                        if (options.client_auth) |auth| {
                                            const scheme = client_scheme.?;
                                            const context_string = " " ** 64 ++ "TLS 1.3, client CertificateVerify\x00";
                                            const signed = context_string ++ p.transcript_hash.peek();
                                            var signature_buf: [PrivateKey.max_signature_len]u8 = undefined;
                                            const signature = try signWith(auth.key, scheme, signed, sign_entropy, &signature_buf);
                                            var verify_head: [8]u8 = undefined;
                                            verify_head[0] = @intFromEnum(tls.HandshakeType.certificate_verify);
                                            mem.writeInt(u24, verify_head[1..4], @intCast(4 + signature.len), .big);
                                            mem.writeInt(u16, verify_head[4..6], @intFromEnum(scheme), .big);
                                            ...
                                            p.transcript_hash.update(&verify_head);
                                            p.transcript_hash.update(signature);
                                            try sealer.add(&verify_head);
                                            try sealer.add(signature);
                                        }
                                        try sealer.seal();
                                    }
                                    const finished_hash = p.transcript_hash.finalResult();
                                    const verify_data = tls.hmac(P.Hmac, &finished_hash, pv.client_finished_key);
                                    ...
                                    const nonce = sequenceNonce(P, pv.client_handshake_iv, handshake_seq);
